  ...
  try {
    ...
    Spreads.findOne({
      ...
      if (removeGamesAfterStart) {
        ...
        const currentTime = new Date(); // Filter out games that have started

        spreadList = spreadList.filter(game => {
          const gameDate = new Date(game.date);
          return (isSameDay(gameDate, startDate) || isAfter(gameDate, startDate)) && (isSameDay(gameDate, endDate) || isBefore(gameDate, endDate) && isBefore(currentTime, gameDate));
        });
      }
